...
export default function BuilderPage() {
  ...
  return <main>
    <nav className="nav"><div className="container nav-inner"><Link className="logo" href="/">RizProof</Link><div className="nav-links"><Link href="/examples">Examples</Link><Link className="pill" href="/">Home</Link></div></div></nav>
    <section className="container section"><div className="section-title"><div><p className="kicker">local-first MVP</p><h2>Proof Pack Builder</h2></div><p className="kicker">Isi data project, tempel bukti, lalu export dossier untuk reviewer.</p></div>
      <div className="builder"><div className="panel"><button type="button" className="btn primary" onClick={() => setData(sample)}>Load sample</button>
        ...
        <div className="panel"><div className="hero-actions" style={{marginTop: 0}}><button type="button" className={mode === "pack" ? "btn primary" : "btn"} onClick={() => setMode("pack")}>Proof Pack</button><button type="button" className={mode === "answer" ? "btn primary" : "btn"} onClick={() => setMode("answer")}>Grant Answer</button><button type="button" className="btn" onClick={copy}>{copied ? "Copied" : "Copy"}</button><button type="button" className="btn" onClick={download}>Download MD</button></div><pre className="panel output">{output}</pre></div></div>
    </section>
  </main>
}
